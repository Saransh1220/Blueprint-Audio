  ...
  styles: [
    `
      ...
      @media (max-width: 768px) {
        .upload-section {
          padding-bottom: 100px !important; // space for mobile tab bar
        }
      }
    `,
  ],
...
export class StudioUploadComponent implements OnInit {
  ...
  onFileSelected(event: Event, type: 'cover' | 'preview' | 'wav' | 'stems') {
    ...
    if (input.files && input.files.length > 0) this.handleFile(input.files[0], type);
  }
  ...
  private handleFile(file: File, type: 'cover' | 'preview' | 'wav' | 'stems') {
    ...
    if (file.size > limits[type].size) {
      this.toastService.show(`File too large. Max ${limits[type].label}`, 'error');
      return;
    }
    switch (type) {
      case 'cover':
  ...
  submitUpload() {
    ...
    const enabledLicenses = formValue.licenses
      ...
      .map((l: any) => ({ ...l, price: Number(l.price) }));
    ...
    const metadata = {
      ...
      tags: formValue.tags,
      genres: [
        { name: formValue.genre, slug: formValue.genre?.toLowerCase().replace(/\s+/g, '-') },
      ],
    ...
    this.labService.createSpec(formData).subscribe({
      next: (event: any) => {
        if (event.type === HttpEventType.UploadProgress && event.total) {
          ...
          // Redirect to Studio tracks tab instead of old dashboard
          this.router.navigate(['/studio/tracks']);
        }
      },
      error: (err: any) => {
